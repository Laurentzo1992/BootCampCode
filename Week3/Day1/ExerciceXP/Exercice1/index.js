...
//2
let li_value = document.getElementsByTagName('li')
for(i=0; i<li_value.length; i++){
     if (li_value[i].innerHTML=="Pete"){
        li_value[i].innerHTML="Richard"
    }
 }
//3
for(let i=0; i<li_value.length; i++){
    li_value[i].innerHTML="Laurent"
 }
//4

let ul_value = document.getElementsByTagName('ul')
for(let i=0; i<ul_value.length; i++){
     if (ul_value[i].innerHTML=="Sarah"){
         ul_value[1].removeChild(li_value[i])
     }
 }
...
let textNode = document.createTextNode('Deconnexion')
newLi.appendChild(textNode)

let ajout = document.getElementById('socialNetworkNavigation').children[0]
ajout.appendChild(newLi)
